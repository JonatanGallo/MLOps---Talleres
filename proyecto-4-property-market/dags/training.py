...
from training_app.dataController import save_all_clean_data, store_raw_data, clear_all_data, store_raw_data_columns
...
with DAG (dag_id="training_dag",
    ...
    schedule_interval=timedelta(minutes=6, seconds=0),   # every 5 minutes and 20 seconds
    ...
    store_raw_data_columns_task = PythonOperator(
        task_id="store_raw_data_columns_task",
        python_callable=store_raw_data_columns,
    )
    ...
    branch >> clear_all_data_task
    branch >> skip_first_time

    clear_all_data_task >> store_raw_data_columns_task >> join_after_branch
    skip_first_time >> join_after_branch

    join_after_branch >> mark_done

    mark_done >> store_raw_data_task >>store_clean_data_task >> train_model >> pause_dag_task
